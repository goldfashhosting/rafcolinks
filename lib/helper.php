<?php
/**
 * RAFCO Link Creator - Helper Module
 *
...
if ( ! class_exists( 'RAFCOCreator_Helper' ) ) {

// Start up the engine
class RAFCOCreator_Helper
{
	 ...
	 */
	public static function get_rafco_option( $key = '' ) {

		// fetch the data
		$data   = get_option( 'rafco_options' );
	...
	/**
	 * get a post meta item with RAFCO data
	 *
	 ...
	 * @return [type]            [description]
	 */
	public static function get_rafco_meta( $post_id = 0, $key = '_rafco_url', $fallback = false ) {
	...
	/**
	 * get the post types that RAFCO is enabled for
	 *
	 * @return [type] [description]
	 */
	 ...
	 * @return [type] [description]
	 */
	public static function get_rafco_status( $action = '' ) {

		// return only publish for saving
		if ( ! empty( $action ) && $action == 'save' ) {
			return apply_filters( 'rafco_post_status', array( 'publish' ), $action  );
		}

		// return the default
		return apply_filters( 'rafco_post_status', array( 'publish', 'future' ), $action );
	}
	 ...
	 * @return [type]           [description]
	 */
	public static function get_rafco_keyword( $post_id = 0 ) {

		// check for a keyword
		$keywd  = get_post_meta( $post_id, '_rafco_keyword', true );
	 ...
	 * @return [type]      [description]
	 */
	public static function get_rafco_api_data( $key = '' ) {

		// fetch the stored option array
		$option = self::get_rafco_option();
	...
	/**
	 * get all the post IDs that contain the RAFCO url
	 *
	 * @return array the post IDs containing the meta key
	 */
	public static function get_rafco_post_ids( $key = '_rafco_url' ) {
	...
	/**
	 * make a API request to the RAFCO server
	 *
	 ...
	 * @return [type]         [description]
	 */
	public static function run_rafco_api_call( $action = '', $args = array(), $user = true, $format = 'json', $decode = true ) {
		...
		if ( empty( $action ) ) {
			return array(
				...
				'message'   => __( 'No API action was provided.', 'rafcolinks' )
			);
		}
		...
		if ( ! in_array( $action, array( 'shorturl', 'expand', 'url-stats', 'stats', 'db-stats' ) ) ) {
			return array(
				...
				'message'   => __( 'The API action was invalid.', 'rafcolinks' )
			);
		}

		// bail if the API key or URL have not been entered
		if(	false === $apikey = self::get_rafco_api_data( 'key' ) ) {
			return array(
				'success'   => false,
				'errcode'   => 'NO_API_DATA',
				'message'   => __( 'No data was returned from the API call.', 'rafcolinks' )
			);
		}

		// only fire if user has the option
		if( false !== $user && false === $check = RAFCOCreator_Helper::check_rafco_cap( 'ajax' ) ) {
			return array(
				'success'   => false,
				'errcode'   => 'INVALID_USER',
				'message'   => __( 'The user requesting the URL does not have authorization.', 'rafcolinks' )
			);
		}
		...
		$build  = wp_remote_post( self::get_rafco_api_url(), array(
			'method'       => 'POST',
			'timeout'      => 45,
			'redirection'  => 5,
			'sslverify'    => false,
		...
		if ( empty( $build ) ) {
			return array(
				...
				'message'   => __( 'The response from the API was empty.', 'rafcolinks' )
			);
		}
		...
		if ( $code !== 200 ) {
			return array(
				...
				'message'   => sprintf( __( 'The API call returned a %s response code.', 'rafcolinks' ), $code )
			);
		}
		...
		if ( empty( $body ) ) {
			return array(
				'success'   => false,
				'errcode'   => 'EMPTY_BODY',
				'message'   => __( 'No data was present in the body from the API call.', 'rafcolinks' )
			);
		}
		...
		if ( empty( $data ) ) {
			return array(
				'success'   => false,
				'errcode'   => 'EMPTY_JSON',
				'message'   => __( 'The JSON could not be parsed.', 'rafcolinks' )
			);
		}
	...
	public static function get_single_shorturl( $post_id = 0, $check = 'sav' ) {
		...
		if ( ! in_array( get_post_type( $post_id ), self::get_rafco_types() ) ) {
			return;
		}

		// bail if the API key or URL have not been entered
		if(	false === $api = self::get_rafco_api_data() ) {
			return;
		}

		// bail if user hasn't checked the box
		if ( false === $onschd = self::get_rafco_option( $check ) ) {
		   	return;
		}

		// check for a link and bail if one exists
		if ( false !== $exist = self::get_rafco_meta( $post_id ) ) {
			return;
		}
	...
	public static function get_single_click_count( $post_id = 0 ) {
		...
		// get the URL
		$url    = self::get_rafco_meta( $post_id );
		...
		// make the API call
		$build  = self::run_rafco_api_call( 'url-stats', array( 'shorturl' => esc_url( $url ) ), false );
		...
		if ( empty( $build ) || false === $build['success'] ) {
			return array(
				'success'   => false,
				'errcode'   => 'NO_DATA',
				'message'   => __( 'No API data was returned.', 'rafcolinks' )
			);
		}
	 ...
	 * @return [type]       [description]
	 */
	public static function filter_rafco_import( $group = array() ) {
	...
	public static function maybe_import_link( $data = array() ) {
		...
		if ( ! empty( $post ) && ! empty( $post[0] ) ) {
			update_post_meta( absint( $post[0] ), '_rafco_url', esc_url( $data['short'] ) );
			update_post_meta( absint( $post[0] ), '_rafco_clicks', absint( $data['clicks'] ) );
		}
	...
	/**
	 * get the link box when we dont have a RAFCO link
	 *
	 * @param  string $link [description]
	 * @return [type]       [description]
	 */
	public static function get_rafco_subbox( $post_id = 0 ) {
		...
		// display the box
		$box   .= '<p class="rafco-meta-block rafco-input-block">';

			// input field for the optional keyword
			$box   .= '<input id="rafco-keyw" class="rafco-keyw" size="20" type="text" name="rafco-keyw" value="' . esc_attr( $keywd ) . '" tabindex="501" />';

			// simple instruction
			$box   .= '<span class="description">' . __( 'optional keyword', 'rafcolinks' ) . '</span>';

		// first check our post status
		if ( ! in_array( get_post_status( $post_id ), array( 'publish', 'future', 'pending' ) ) ) {
			$box   .= '<p class="yourls-meta-block howto">' . __( 'a RAFCO link cannot be generated until the post is saved.', 'rafcolinks' ) . '</p>';
		} else {
	...
	public static function yourls_submit_box( $post_id = 0 ) {
		...
		$box   .= '<p class="yourls-meta-block yourls-submit-block">';
			...
			$box   .= '<input type="button" class="button button-secondary button-small yourls-api" id="yourls-get" name="yourls-get" value="' . __( 'Create RAFCO link', 'rafcolinks' ) . '" tabindex="502" data-nonce="' . esc_attr( $nonce ) . '" data-post-id="' . absint( $post_id ) . '" />';
	...
	/**
	 * get the link box when we have a RAFCO link
	 *
	 * @param  string  $link    [description]
	 * @param  integer $post_id [description]
	 ...
	 * @return [type]           [description]
	 */
	public static function get_rafco_linkbox( $link = '', $post_id = 0, $count = 0 ) {
		...
		$box   .= '<p class="yourls-meta-block yourls-exist-block">';
			...
			$box   .= '<span class="dashicons dashicons-no yourls-delete" title="' . __( 'Delete Link', 'rafcolinks' ) . '" data-post-id="' . absint( $post_id ) . '" data-nonce="' . esc_attr( $nonce ) . '"></span>';

		$box   .= '</p>';

		// the box with the counting
		$box   .= '<p class="yourls-meta-block howto"> ' . sprintf( _n( 'Your RAFCO link has generated %d click.', 'Your RAFCO link has generated %d clicks.', absint( $count ), 'rafcolinks' ), absint( $count ) ) .'</p>';
	...
	/**
	 * build the inline action row for creating a RAFCO link
	 *
	 * @param  integer $post_id [description]
	 * @return [type]           [description]
	 */
	public static function create_row_action( $post_id = 0 ) {
		...
		// return the link
		return '<a href="#" class="yourls-admin-row-link yourls-admin-create" data-nonce="' . esc_attr( $nonce ) . '" data-post-id="' . absint( $post_id ) . '">' . __( 'Create RAFCO', 'rafcolinks' ) . '</a>';
	}

	/**
	 * build the inline action row for updating a RAFCO link
	 *
	 * @param  integer $post_id [description]
	 * @return [type]           [description]
	 */
	public static function update_row_action( $post_id = 0 ) {
		...
		// return the link
		return '<a href="#" class="yourls-admin-row-link yourls-admin-update" data-nonce="' . esc_attr( $nonce ) . '" data-post-id="' . absint( $post_id ) . '">' . __( 'Update RAFCO', 'rafcolinks' ) . '</a>';
	}
	...
	public static function get_api_status_data() {
		...
		$data   = array(
			...
			'text'  => __( 'The status of the RAFCO API could not be determined.', 'rafcolinks' )
		);
		...
		if ( $check == 'connect' ) {
			...
			$data   = array(
				...
				'text'  => __( 'The RAFCO API is currently accessible.', 'rafcolinks' )
			);
		}
		...
		if ( $check == 'noconnect' ) {
			...
			$data   = array(
				...
				'text'  => __( 'The RAFCO API is currently NOT accessible.', 'rafcolinks' )
			);
		}
	 ...
	 * @return [type]         [description]
	 */
	public static function check_rafco_cap( $action = 'display', $cap = 'edit_others_posts' ) {
	...
	public static function meta_save_check( $post_id = 0, $cap = 'edit_post' ) {
		...
		if ( false === $check = self::check_rafco_cap( 'save' ) ) {
			return true;
		}
